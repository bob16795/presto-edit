...
pub struct FileBuffer {
    ...
    pub highlight: Vec<Vec<lsp::LSPHighlight>>,
}

impl FileBuffer {
    fn update_hi(&mut self, lsp: &mut lsp::LSP) {
        self.highlight = lsp
            .lock()
            .unwrap()
            .get_highlight(self.filename.clone())
            .expect("fdsa");
    }
}
...
impl BufferFuncs for FileBuffer {
    ...
    fn setup_lsp(&mut self, lsp: &mut lsp::LSP) {
        let conts = self.data.join("\n");
        lsp.lock()
            .unwrap()
            .open_file(self.filename.clone(), conts)
            .expect("cant open");
    }
    ...
    fn draw_conts(&self, handle: &mut dyn drawer::Handle, coords: Rect) -> std::io::Result<()> {
        ...
        for idx in 0..coords.h {
            ...
            if (line_idx as usize) < self.highlight.len() {
                hi = &self.highlight[line_idx as usize]
            }
            ...
            for h in hi {
                for idx in (h.pos + 5)..(h.pos + h.len + 5) {
                    if idx < colors.len() {
                        colors[idx] = h.color.clone();
                    }
                }
            }
    ...
    fn event_process(&mut self, ev: event::Event, lsp: &mut lsp::LSP, coords: Rect) {
        ...
        match (self.mode.clone(), ev) {
            ...
            (_, event::Event::Save(None)) => {
                ...
                lsp.lock()
                    .unwrap()
                    .save_file(self.filename.clone(), conts)
                    .unwrap();
            }
